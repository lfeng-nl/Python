...
- 除了赋值时的解包, 也可以在函数或表达式中使用`* 或 **` 对可迭代对象或字典进行解包; 
...
- 包：含有一些模块文件和一个`__init__.py(python2需要, python3 不需要)`（用于表明这个文件夹是特别的）的文件夹，；
...
- `__import__(mode_name)`: import 不支持以字符串形式加载 ,可以使用`__import__` 实现动态指定模块名加载模块; 
...
- 基础：包导入语句的路径中 ==每个目录==内都必须有一个`__init__.py`文件；
  ...
  - python首次导入某个目录时，会自动执行该目录下的`__init__.py`文件中的所有程序代码，
  - 可以在`__init__.py`内使用`__all__`列表来定义目录以from * 语句导入时，需要导入什么；
...
- 也可以使用`imp.load_source('mode_name', 'path')`导入指定模块,而不用加入`sys.path`中;
- 使用模块的`__file__`属性可以查看模块的所在磁盘路径位置;
